    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        var wvclient = new WebViewClient() {
            ...
            public void onPageFinished(WebView view, String url) {
                if (url.startsWith(ServerReviewsAPI.API_URL +"/receiveToken/")) {
                    String token = url.substring((ServerReviewsAPI.API_URL +"/receiveToken/").length());
                    ServerReviews.staticSettings.setString("token", token);
                    Toast.makeText(context, "Successfully Authorized", Toast.LENGTH_SHORT).show();
                    getActivity().onBackPressed();
                } else if (url.contains(ServerReviewsAPI.API_URL + "/error")) {
